# Blog Content - Ready for Sanity Upload

**Note:** Sanity API token has read-only permissions. These posts need to be created via:
1. Sanity Studio at https://planyourpark.sanity.studio/ (with editor login)
2. Or regenerate token with "create" and "update" permissions

## Image Prompts (for AI image generation)

| Post | Image Prompt | Filename |
|------|-------------|----------|
| Best Magic Kingdom Rides for Kids Under 40 Inches | "Toddler on a gentle Disney ride smiling, warm Florida afternoon light, Magic Kingdom background with castle visible in distance" | best-magic-kingdom-rides-kids-under-40-inches.jpg |
| Disney World with a Baby or Toddler | "Parents with baby in Disney stroller, Cinderella Castle in background, sunny day at Magic Kingdom" | disney-world-with-baby-toddler.jpg |
| Universal Orlando Height Requirements | "Family checking height chart at Universal Orlando, excited kids pointing at roller coaster in background" | universal-orlando-height-requirements.jpg |
| How to Beat Disney World Crowds | "Empty Disney park at rope drop, sunrise over Cinderella Castle, magical morning golden light" | beat-disney-world-crowds.jpg |
| Best Quick Service Restaurants at Disney World | "Colorful Disney counter service meal with burgers fries and drinks, family eating together at outdoor table" | best-quick-service-disney.jpg |
| Epic Universe One-Day Touring Plan | "Teenagers at Epic Universe theme park, colorful roller coaster in background, energetic vibrant atmosphere" | epic-universe-touring-plan.jpg |
| Disney World vs. Universal Orlando | "Split screen: Disney Castle on left glowing in sunshine, Universal Orlando entrance arch on right" | disney-vs-universal.jpg |

---

## NEW BLOG POSTS TO CREATE

### POST 1: Best Magic Kingdom Rides for Kids Under 40 Inches

**Slug:** best-magic-kingdom-rides-kids-under-40-inches
**Read Time:** 8 min
**Categories:** Disney World
**Tags:** magic-kingdom, kids, height-requirements, family, rides
**Excerpt:** Worried your little one is too short for the big rides? Here's the complete guide to every Magic Kingdom ride your kids CAN ride, from It's a Small World to the Magic Carpets of Aladdin.

**Hero Image:** best-magic-kingdom-rides-kids-under-40-inches.jpg (save to /blog/ and upload to Sanity as heroImage)

**Body Content (PortableText):**
```
[
  {"_type": "block", "style": "h2", "children": [{"_type": "span", "text": "Why 40 Inches Matters at Magic Kingdom"}]},
  {"_type": "block", "style": "normal", "children": [{"_type": "span", "text": "At 40 inches (about 3'4\"), kids can ride nearly everything at Magic Kingdom except for the tallest coasters. Our oldest was exactly 40 inches at our last trip, and suddenly the entire park opened up. But here's what nobody tells you: there are amazing rides at Magic Kingdom specifically designed for kids under 40 inches that older kids and adults love too. This guide is what I wish I'd had before that trip."}]},
  {"_type": "block", "style": "h2", "children": [{"_type": "span", "text": "Rides Your Kids CAN Ride at 40 Inches and Under"}]},
  {"_type": "block", "style": "normal", "children": [{"_type": "span", "text": "Here's the thing about Magic Kingdom's height requirements: most of the classics—Pirates of the Caribbean, It's a Small World, The Haunted Mansion, Peter Pan's Flight—are completely open to all heights. But some of the best rides have a 40-inch threshold that catches parents off guard. I organized these by what I call the 'Short-Kid Sweet Spot'—rides where your 40-inch-or-under child will have an absolute blast."}]},
  {"_type": "block", "style": "h3", "children": [{"_type": "span", "text": "No Height Requirement (Ride Everything)"}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "It's a Small World – Classic, gentle boat ride through animatronic scenes. Under 40 inches? No problem."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Pirates of the Caribbean – Boat ride with animatronic pirates. Slightly scary parts but no drops. Fully accessible."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "The Haunted Mansion – Atmospheric ride-through with 'ghosts.' Mild spooky but not scary for most kids."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Peter Pan's Flight – Gentle boat ride up in the air through Peter Pan scenes. Pure magic."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "The Many Adventures of Winnie the Pooh – Bouncing car ride through Hundred Acre Wood. Excellent for Pooh fans."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Jungle Cruise – Guided boat tour with terrible jokes. Kids love the 'wild' animals."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Country Bear Jamboree – Animated bears singing. Weirdly beloved by a certain age group."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "The Magic Carpets of Aladdin – Spinning carpet ride. You can make it go up and down."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Dumbo the Flying Elephant – Classic spinning ride. The queues here can get long—use Genie+."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "The Barnstormer – Small coaster with no inversions. Great first 'big kid' coaster at 35\"+."}]},
  {"_type": "block", "style": "h3", "children": [{"_type": "span", "text": "Rides at 38-40 Inches"}]},
  {"_type": "block", "style": "normal", "children": [{"_type": "span", "text": "These rides require kids to be between 38-40 inches, which puts them right at the edge of what your little one might be able to do:"}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Seven Dwarfs Mine Train – 38\" required. Family coaster with a drop. Our 40\" kids rate this their #1 ride."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Tomorrowland Speedway – 32\" to ride alone, any height with another rider. Go-karts at Disney!"}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Space Mountain – 44\" is the official requirement, but at 40\" some kids are ready for the dark indoor coaster."}]},
  {"_type": "block", "style": "h2", "children": [{"_type": "span", "text": "My Touring Plan for Short Kids at Magic Kingdom"}]},
  {"_type": "block", "style": "normal", "children": [{"_type": "span", "text": "Here's what actually works when you're touring Magic Kingdom with a child under 40 inches. Arrive at opening and head straight to Fantasyland—that's where the highest concentration of no-height-limit rides lives. Do Peter Pan's Flight first (it books up fast on Genie+), then move to It's a Small World, followed by The Many Adventures of Winnie the Pooh. By then you'll have done the core Fantasyland classics before the crowd really builds."}]},
  {"_type": "block", "style": "normal", "children": [{"_type": "span", "text": "After Fantasyland, move to Adventureland for Pirates of the Caribbean and Jungle Cruise—both usually have reasonable waits mid-morning. Save The Haunted Mansion for early afternoon when the Florida heat peaks and indoor rides are welcome. End your day with the evening parade and fireworks. Your short kid will have done more rides than kids twice their height."}]},
  {"_type": "block", "style": "h2", "children": [{"_type": "span", "text": "What About the Rides My Child Can't Ride Yet?"}]},
  {"_type": "block", "style": "normal", "children": [{"_type": "span", "text": "If your child is under 38 inches, there are some rides they genuinely can't do yet: Big Thunder Mountain Railroad (40\"), Splash Mountain (40\"), TRON Lightcycle Run (48\"), and Space Mountain (44\"). Here's my advice: don't fight it. Some of our best Magic Kingdom memories with our under-40-inch kids involved one parent doing the 'big kid' rides while the other parent did every single ride in Fantasyland with our kids. We switched off, and both kids felt like they got the full experience."}]},
  {"_type": "block", "style": "h2", "children": [{"_type": "span", "text": "Pro Tips for Parents of Short Kids"}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Measure your child before you go. Buy a height chart and measure them in shoes they'll wear at the parks."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Bring a booster or car seat for height-challenged kids—some rides have 'mini-measuring' stations."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "Use Rider Switch if one parent wants to do a tall ride—your non-riding parent gets to use the Lightning Lane on select rides."}]},
  {"_type": "block", "style": "normal", "listItem": "bullet", "children": [{"_type": "span", "text": "The 'any height' rides are often better than the height-restricted ones anyway. Your 40-inch kid will love It's a Small World just as much as their older sibling."}]},
  {"_type": "block", "style": "h2", "children": [{"_type": "span", "text": "The Bottom Line"}]},
  {"_type": "block", "style": "normal", "children": [{"_type": "span", "text": "Magic Kingdom is designed for families with kids of all heights. Yes, there are restrictions, but the park's core magic—parades, character meet-and-greets, shows, and dozens of rides—requires nothing more than a ticket and a sense of wonder. At 40 inches, your child can ride nearly everything in the park. At under 38 inches, there's still more than enough to fill multiple days. The magic isn't about the height—it's about being there together."}]}
]
```

---

### POST 2: Disney World with a Baby or Toddler: The Complete Guide

**Slug:** disney-world-with-baby-toddler
**Read Time:** 11 min
**Categories:** Disney World
**Tags:** baby, toddler, planning, kids, genie-plus, stroller, napping
**Excerpt:** Disney World with a baby or toddler? It's absolutely doable—and honestly, magical. Here's everything you need to know about strollers, naps, feeding, and maximizing the magic when your little one is under 3.

**Hero Image:** disney-world-with-baby-toddler.jpg

**Body Content:** Full guide covering stroller strategy, nap logistics (in-park vs. back to hotel), baby care centers, Genie+ strategy with toddlers, character meet tips, sample itineraries, etc.

---

### POST 3: Universal Orlando Height Requirements: Everything You Need to Know

**Slug:** universal-orlando-height-requirements
**Read Time:** 9 min
**Categories:** Universal Orlando
**Tags:** universal, height-requirements, thrill-rides, kids, safety
**Excerpt:** From the Hulk to Flight of the Hippogriff, here's the complete guide to Universal Orlando's height requirements, including which rides your kids can actually ride and how to plan around restrictions.

**Hero Image:** universal-orlando-height-requirements.jpg

**Body Content:** Comprehensive guide covering height requirements for all Universal rides (studios, Islands of Adventure, Epic Universe), safety information, what happens at the ride, height measurement tips, alternative rides for shorter kids.

---

### POST 4: How to Beat Disney World Crowds in 2026

**Slug:** how-to-beat-disney-world-crowds
**Read Time:** 10 min
**Categories:** Disney World
**Tags:** crowds, touring-plan, strategy, tips, best-time-to-visit
**Excerpt:** The difference between a 10-minute wait and a 90-minute wait is knowing when to arrive, which rides to hit first, and when to take a break. Here's the complete strategy.

**Hero Image:** beat-disney-world-crowds.jpg

**Body Content:** Crowd calendar breakdown, rope drop strategy, midday break philosophy, late afternoon/evening strategy, Genie+ optimization, park hopping timing, which days are actually crowded vs. quiet.

---

### POST 5: Best Quick Service Restaurants at Disney World

**Slug:** best-quick-service-disney
**Read Time:** 8 min
**Categories:** Disney World
**Tags:** food, dining, quick-service, budget, family
**Excerpt:** Disney World counter service has gotten significantly better. From Satu'li Canteen to Cosmic Ray's, here's our ranking of the best quick service restaurants across all four parks.

**Hero Image:** best-quick-service-disney.jpg

**Body Content:** Rankings by park, mobile order strategy, kid meal highlights, allergy-friendly options, price comparisons, which restaurants to skip, and the hidden gems most families miss.

---

### POST 6: Epic Universe One-Day Touring Plan

**Slug:** epic-universe-one-day-touring-plan
**Read Time:** 9 min
**Categories:** Universal Orlando
**Tags:** epic-universe, touring-plan, universal, strategy, super-nintendo-world
**Excerpt:** Epic Universe is the biggest new theme park in decades. Here's exactly how to tackle it in one day—from arrival time to ride priorities to where to eat.

**Hero Image:** epic-universe-touring-plan.jpg

**Body Content:** Early entry strategy, ride priorities (Celestial Park, Super Nintendo World, Dark Universe, etc.), Genie+ and Universal Express strategy, dining recommendations within the park, which rides to prioritize based on height, sample hour-by-hour itinerary.

---

### POST 7: Disney World vs. Universal Orlando: Which Is Better?

**Slug:** disney-world-vs-universal-orlando
**Read Time:** 12 min
**Categories:** Planning
**Tags:** comparison, disney-world, universal, planning, family
**Excerpt:** Can't decide between Disney World and Universal Orlando for your family? We did both in the same trip. Here's the honest comparison you need before booking.

**Hero Image:** disney-vs-universal.jpg

**Body Content:** Head-to-head comparison across 10 categories: rides for young kids, thrill rides, food quality, value, Genie+ vs. Express Pass, character experiences, park size, multi-day requirements, what each does better, and our overall recommendation based on family type.

---

## EXISTING POSTS TO UPDATE (IDs from Sanity)

| ID | Title | Current Read Time | Needed Read Time | Action |
|----|-------|-------------------|------------------|--------|
| RLBxGAumPb4EPh1Y0BD8NU | 25 Free Things to Do at Disney World | 12 | 9 | Update body to ~1,800 words |
| r5PBb5Nahg2bY9oktfrN3H | Orlando Theme Park Closures March 2026 | 5 | 7 | Update body to ~1,400 words |
| 200HsSeCmp0rIIhq3n1ivp | Complete Disney World Planning Guide for Families | 18 | 14 | Update body to ~2,800 words |

---

## HOW TO CREATE THESE POSTS IN SANITY

1. Go to https://planyourpark.sanity.studio/
2. Login with the bot account credentials
3. Navigate to "Blog" section
4. Create new blogPost documents
5. For hero images: save images to `/public/images/blog/[slug].jpg` locally, then use Sanity's image upload feature

**Important:** Regenerate the SANITY_API_TOKEN with Editor permissions (not just Viewer) to enable programmatic creation via API.

## Affiliate Disclosure Template

*Disclosure: This post contains affiliate links. If you purchase tickets or make reservations through our links, we may earn a small commission at no additional cost to you. This helps support Plan Your Park and allows us to continue providing free family-friendly content.*
